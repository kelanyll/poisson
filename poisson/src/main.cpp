...
ULDataFrame get_data() {
    std::vector<std::vector<std::string>> data{read_csv("prem-11-12.csv")};

    auto convert_str_vec_to_uint{[](std::vector<std::string>& str_vec) {
    ...
    df.load_data(std::move(index),  
        ...
        std::make_pair("FTHG", convert_str_vec_to_uint(data[4])),
        std::make_pair("FTAG", convert_str_vec_to_uint(data[5]))
    );
